...
final class MosquitoRiskTest: XCTestCase {
    ...
    // Caso 9: Temperatura justo debajo del umbral alto pero en rango medio
    func testTemperatureJustBelowHighThreshold() {
        let weather = WeatherData(
            temperature: 21.9, // justo debajo de 22, pero en rango 18-22
            humidity: 65,       // ≥50, cumple para medio
            condition: "Sunny",
            precipitation: 0,
            windSpeed: 10,
            sunrise: Date(),
        ...
        let risk = MosquitoRisk.calculateRisk(from: weather)
        XCTAssertEqual(risk, .medium, "Esperaba riesgo MEDIO (temp 18-22, humidity ≥50) pero obtuve \(risk)")
    }
    ...
    // Caso 11: Viento extremo con condiciones de alto riesgo
    func testExtremeWindOverridesHighRisk() {
        let weather = WeatherData(
            temperature: 35,  // Muy alto
            humidity: 90,     // Muy alto
            condition: "Rain",
            precipitation: 15, // Lluvia intensa
            windSpeed: 45,    // Viento extremo ≥40
            sunrise: Date(),
            sunset: Date()
        )

        let risk = MosquitoRisk.calculateRisk(from: weather)
        XCTAssertEqual(risk, .low, "Viento ≥40 km/h debería forzar riesgo BAJO independientemente de otros factores")
    }

    // Caso 12: Valores extremos de temperatura
    func testExtremeTemperatures() {
        // Temperatura muy baja
        let coldWeather = WeatherData(
            temperature: 5,
            humidity: 80, // Humedad alta pero temperatura muy baja
            condition: "Clear",
            precipitation: 0,
            windSpeed: 10,
            sunrise: Date(),
        ...
        let coldRisk = MosquitoRisk.calculateRisk(from: coldWeather)
        XCTAssertEqual(coldRisk, .low, "Temperatura muy baja debería resultar en riesgo BAJO")
        ...
        let hotRisk = MosquitoRisk.calculateRisk(from: hotWeather)
        XCTAssertEqual(hotRisk, .high, "Temperatura muy alta (40°C) con humedad ≥60 debería ser riesgo ALTO")
    }
}
